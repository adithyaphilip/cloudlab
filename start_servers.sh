...
parallel-ssh -x "-o StrictHostKeyChecking=no -i ~/.ssh/id_rsa" -h servers_file_pssh "sudo tc qdisc del dev eno50 root; sudo tc qdisc add dev eno50 root netem delay 20ms;"
