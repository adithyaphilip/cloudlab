if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ] || [ -z "$4" ] || [ -z "$5" ] || [ -z "$6" ] || [ -z "$7" ]
  then
    echo "Usage: exp_set.sh test_time congestion_algo num_nodes_total_per_side total_times_repeat type_2_netem_delay_ms variant if_name"
    exit 1
fi
  ...
  do
    ...
    do
      ...
      do
        ...
        ./rerun_exp.sh $flow_per_node $1 $2 $nodes $3 $4 $base_delay $5 $delayed_count $6 $7
      done
    done
  done
done
