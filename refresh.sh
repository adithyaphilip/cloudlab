...
parallel-ssh $PSSH_OPTIONS -h setup_cl_clients "sudo apt remove -y iperf3; rm -rf iperf; cd ~; git clone https://github.com/adithyaphilip/iperf.git && cd iperf && ./bootstrap.sh && ./configure && make && cd .. && sudo rm -f /usr/local/bin/iperf3 /usr/bin/iperf3 && sudo ln -s $(pwd)/iperf/src/iperf3 /usr/local/bin/iperf3 && sudo ln -s $(pwd)/iperf/src/iperf3 /usr/bin/iperf3"
